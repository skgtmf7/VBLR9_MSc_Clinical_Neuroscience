...
# Load AD GWAS dataset (Bellenguez et al., 2022)
...
gwas<-read.table("Bellenguez_subset.tsv", header = TRUE, stringsAsFactors = FALSE, fill = TRUE)
# 21,101,114 SNPs
colnames(gwas)
...
#147,118 out of 148,826 blood mQTLs are found in the GWAS

# save filtered gwas dataframe
write.csv(gwas_4, "filtered_gwas.csv", row.names = FALSE)

# save filtered mqtl foreground
write.csv(mqtl_foreground, "fetal_mqtl_foreground.csv", row.names = FALSE)
...
#10,092 fetal mQTLs are found in the GWAS
#5,343 PFC mQTLs are found in the GWAS
#4,669 CER mQTLs are found in the GWAS
#4,904 STR mQTLs are found in the GWAS

#save filtered (non clumped) mqtl foreground
write.csv(nonclumped_mqtl_foreground, "fetal_nonclumped_mqtl_foreground.csv", row.names = FALSE)
