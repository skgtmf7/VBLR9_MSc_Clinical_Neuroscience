...
gwas_all <- fread("/Documents/enrichment_analysis/Longevity_input/longevity_filtered_gwas.csv")
# 11,507,096

# Load in mQTL FOREGROUND
gwas_qtl <- read.csv("/Documents/enrichment_analysis/longevity_input/fetal_longevity_mqtl_foreground.csv")
...
# Save Results
save(perms.tmp, true, pThres, file = "permutation_results_longmqtl.RData")
write.csv(rbind(pThres, true,qtl.rate, all.rate, emp), "Fetal_enrichRESULT_longmqtl.csv")
...
# Run analysis with HPC.R script and 10 permutations in local R, 
...
# Save list of SNPs
write.csv(mqtl_snps_df, "nonclumped_longevity_mQTL_hits.csv")
